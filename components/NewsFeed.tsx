...
export async function NewsFeed() {
    ...
    if (error || !news) {
        return <div className="text-destructive text-sm">Failed to load news.</div>
    }
                ...
                {news.map((item: any, i: number) => (
                    <div key={i} className="border-b last:border-0 pb-4 last:pb-0">
                        <Link
                            href={item.link || "#"}
                            target="_blank"
                            className="font-semibold text-primary hover:text-primary-hover hover:underline block"
                        >
                            {item.title}
                        </Link>
