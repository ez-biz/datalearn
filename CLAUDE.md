...
- **Validation flow** — `validateSubmission(slug, userResult, dialect)` server action compares user rows against `SQLProblem.expectedOutputs[<dialect>]` (JSON), falling back to the legacy `expectedOutput` field when missing (transition window through v0.5.1). Comparator is `lib/sql-validator.ts`. Also writes a `Submission` row when the user is authed.
- **Per-dialect canonical solutions** (v0.5.0+) — `SQLProblem.solutions: Json` and `SQLProblem.expectedOutputs: Json` are maps keyed by `Dialect`. Every entry of `dialects[]` should have a corresponding key in both maps. `solutionSql` and `expectedOutput` (singular) remain as legacy fallback fields and will be dropped in v0.5.1. Audit current data with `npm run audit:dialects`.
